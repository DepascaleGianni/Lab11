...
class DAO():
    ...
    def get_weight(a,p1,p2):
        ...
        query = """select count(*)
                from
                (select distinct pv.data1
                from
                (select gds1.Product_number as p1, gds1.`Date` as data1, gds1.Retailer_code as r1, gds2.Product_number as p2,gds2.`Date` as data2, gds2.Retailer_code as r2
                from go_daily_sales gds1 , go_daily_sales gds2
                where gds2.`Date` = gds1.`Date` and gds2.Retailer_code = gds1.Retailer_code and gds1.Product_number < gds2.Product_number and 
                year(gds1.`Date`) = %s) as pv
                where (pv.p1= %s and pv.p2 = %s) or  (pv.p1 = %s and pv.p2 =%s)
                ) as tot
                """
